...
begin
    ...
    begin
        if (load_byte = '0') then   -- word addressing
            var_addr := conv_integer(addr_in(3 downto 0));
        else                -- byte addressing
            var_addr := conv_integer(addr_in(4 downto 1));
        end if;
        ...
        if (reset = '1') then
            ...
            var_data_mem(5)  := X"1122";
            var_data_mem(6)  := X"00ff";
        ...
        if (load_byte = '1') then
            ...
            -- accessing lower and upper byte according to LSB of address
            if (addr_in(0) = '0') then
                data_out <= "00000000" & var_data_mem(var_addr)(7 downto 0);
            else
                data_out <= "00000000" & var_data_mem(var_addr)(15 downto 8);
            end if;
        else
